...
class MarketDataService {
  ...
  async getCommodityData(commodityId, options = {}) {
    try {
      ...
      const results = await db
        .collection(this.collections.prices)
        .aggregate(pipeline)
        .toArray();
  ...
  async calculateTradingRoutes(startSystem, options = {}) {
    try {
      ...
      const nearbySystemsMap = await this.getNearbySystemsWithCoordinates(
        startSystem,
        maxJumps
      );
      ...
      if (nearbySystemNames.length === 0) {
        return {
          ...
          routes: [],
          message:
            'No nearby systems found or start system coordinates unavailable',
        };
      }

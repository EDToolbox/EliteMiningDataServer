...
- `GET /api/status/endpoints` - API Dokumentation

### Mining Daten
- `GET /api/mining/sites/:systemName` - Mining-Standorte in einem System
- `GET /api/mining/reports` - Aktuelle Mining-Berichte
- `GET /api/mining/stats` - Mining-Statistiken
- `GET /api/mining/opportunities/:systemName` - Mining-Möglichkeiten in der Nähe
- `GET /api/mining/hotspots` - Hotspot-Informationen

### Sternsysteme
- `GET /api/systems/:systemName` - System-Informationen
- `GET /api/systems/search/:searchTerm` - Systeme suchen
- `GET /api/systems/:systemName/nearby` - Nahe Systeme
- `GET /api/systems/:system1/distance/:system2` - Entfernung zwischen Systemen

### Rohstoffe und Preise
- `GET /api/commodities/:commodityName/sell` - Beste Verkaufspreise
- `GET /api/commodities/:commodityName/buy` - Beste Kaufpreise
- `GET /api/commodities/mining/overview` - Mining-Rohstoffe Übersicht
- `GET /api/commodities/:commodityName/history` - Preisverlauf

## WebSocket API

Verbinde zu `ws://localhost:3000` für Echtzeit-Updates.
...
**Verfügbare Kanäle:**
- `mining` - Mining-relevante EDDN Daten
- `eddn` - Alle EDDN Daten
- `commodities` - Rohstoffpreis-Updates
...
- Live-Stream von Spieldaten über ZeroMQ
- Automatische Filterung auf Mining-relevante Events
- Anonymisierte Spielerdaten

### Inara API
- Marktdaten und Preise
- Stations-Informationen
- Rohstoff-Preisvergleiche

### EDSM (Elite Dangerous Star Map)
- System-Koordinaten
- Sterne und Planeten-Daten
...
├── services/         # Services (Database, Logger, etc.)
├── server.js         # Haupt-Server Klasse
└── index.js          # Entry Point
```

### Logging

Das System verwendet Winston für strukturiertes Logging:
- `logs/combined.log` - Alle Logs
- `logs/error.log` - Nur Fehler
- Console Output in Entwicklung
...
```ini
[Unit]
Description=Elite Mining Data Server
After=network.target

[Service]
Type=simple
User=eliteuser
WorkingDirectory=/opt/elite-mining-server
...
- EDDN Nachrichten pro Sekunde
- Datenbank Performance
- Speicherverbrauch

### Gesundheitschecks
- `GET /health` für Load Balancer
- `GET /api/status` für detaillierte Informationen
...
| `DB_PATH` | SQLite Datenbankpfad | ./data/mining_data.db |
| `EDDN_RELAY_URL` | EDDN ZeroMQ URL | tcp://eddn.edcd.io:9500 |
| `INARA_API_KEY` | Inara API Schlüssel | - |
| `EDSM_API_KEY` | EDSM API Schlüssel | - |
| `LOG_LEVEL` | Log Level | info |
| `CACHE_DURATION_MINUTES` | Cache Dauer | 15 |

## Performance

### Optimierungen
- SQLite mit Indexierung
- In-Memory Caching
- Komprimierte HTTP Responses
- WebSocket Heartbeat
- Rate Limiting (konfigurierbar)

### Skalierung
- Stateless Design
- Externe Datenbank möglich
...
### Inara API
1. Registriere dich auf [Inara.cz](https://inara.cz)
2. Gehe zu Account Settings > API
3. Erstelle einen neuen API Key
4. Trage ihn in `.env` als `INARA_API_KEY` ein

### EDSM API
1. Registriere dich auf [EDSM.net](https://www.edsm.net)
2. Gehe zu Account Settings > API
3. Erstelle einen API Key
4. Trage ihn in `.env` als `EDSM_API_KEY` ein

## Troubleshooting

### Häufige Probleme

**EDDN Verbindung fehlschlägt:**
- Überprüfe Firewall-Einstellungen
- ZeroMQ Port 9500 muss erreichbar sein
- Prüfe Logs für Verbindungsfehler

**Hoher Speicherverbrauch:**
- Reduziere `CACHE_DURATION_MINUTES`
- Überprüfe `MAX_CACHE_ENTRIES`
- Aktiviere Log Rotation

**API Rate Limits:**
- Erhöhe Delays zwischen Requests
- Prüfe API Key Limits
...
- Discord: [Elite Dangerous Community]
- Reddit: r/EliteDangerous
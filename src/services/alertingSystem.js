...
class AlertingSystem extends EventEmitter {
  ...
  sendConsoleNotification(channel, alert) {
    ...
    console.log(
      `${timestamp} ${severityLabel} ALERT: ${alert.title} - ${alert.message}`
    );
    return true;
  }
  ...
  getAlertStatistics(timeRange = '24h') {
    ...
    const resolvedAlerts = recentAlerts.filter(
      (alert) => alert.resolved && alert.resolvedAt
    );
    if (resolvedAlerts.length > 0) {
      ...
      stats.avgResolutionTime = Math.round(
        totalResolutionTime / resolvedAlerts.length / 1000
      ); // seconds
    }
  ...
  updateNotificationChannel(channelName, config) {
    ...
    if (existingChannel) {
      this.notificationChannels.set(channelName, {
        ...existingChannel,
        ...config,
      });
    } else {
  ...
  toCsvFormat(data) {
    const rows = [
      'timestamp,severity,title,message,status,acknowledgedBy,resolvedBy',
    ];

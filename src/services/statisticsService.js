...
class StatisticsService {
  ...
  async getEDDNStatistics() {
    try {
      ...
      const messageStats = await db
        .collection(this.collections.eddnLogs)
        .aggregate([
          {
            $facet: {
              total: [{ $count: 'count' }],
  ...
  async getMiningStatistics() {
    try {
      ...
      const miningStats = await db
        .collection(this.collections.miningLocations)
        .aggregate([
          {
            $facet: {
              total: [{ $count: 'count' }],
      ...
      const result = {
        ...
        analysis: {
          ...
          avgProfitability: this.calculateAverageMiningProfitability(
            stats.profitability
          ),
  ...
  async getAPIUsageStatistics() {
    try {
      ...
      const usageStats = await db
        .collection(this.collections.apiUsage)
        .aggregate([
          {
            $match: { timestamp: { $gte: oneDayAgo } },
          },
  ...
  async getWebSocketStatistics() {
    try {
      ...
      const wsStats = await db
        .collection(this.collections.websocketConnections)
        .aggregate([
          {
            $facet: {
              total: [{ $count: 'count' }],
  ...
  async getSystemStatistics(db) {
    const result = await db
      .collection(this.collections.systems)
      .countDocuments();
    return { total: result };
  }
  ...
  async getStationStatistics(db) {
    const result = await db
      .collection(this.collections.stations)
      .countDocuments();
    return { total: result };
  }
  ...
  async getRecentActivityStatistics(db) {
    ...
    const activity = await db
      .collection(this.collections.marketData)
      .aggregate([
        {
          $match: { timestamp: { $gte: oneHourAgo } },
        },
  ...
  calculateAverageMiningProfitability(profitability) {
    ...
    const total = profitability.reduce(
      (sum, p) => sum + (p.avgProfitability || 0),
      0
    );
  ...
  findSlowestEndpoint(endpoints) {
    ...
    return endpoints.reduce((slowest, current) =>
      current.avgResponseTime > (slowest?.avgResponseTime || 0)
        ? current
        : slowest
    )?._id;
  }
  ...
  findFastestEndpoint(endpoints) {
    ...
    return endpoints.reduce((fastest, current) =>
      current.avgResponseTime < (fastest?.avgResponseTime || Infinity)
        ? current
        : fastest
    )?._id;
  }

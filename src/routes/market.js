...
function calculateMarketVolatility(locations, priceHistory) {
  ...
  const priceVariation = buyPrices.length > 0 ? 
    calculateStandardDeviation(buyPrices) / (buyPrices.reduce((sum, p) => sum + p, 0) / buyPrices.length) : 0;
  
  const supplyVariation = supplies.length > 0 ? 
    calculateStandardDeviation(supplies) / (supplies.reduce((sum, s) => sum + s, 0) / supplies.length) : 0;
  
  const demandVariation = demands.length > 0 ? 
    calculateStandardDeviation(demands) / (demands.reduce((sum, d) => sum + d, 0) / demands.length) : 0;
    ...
    return commodityData.map(commodity => {
      ...
      return {
        ...
        age: {
          ...
          freshness: ageHours < 1 ? 'Very Fresh' : 
                    ageHours < 6 ? 'Fresh' : 
    ...
    return {
      ...
      summary: {
        ...
        averageProfit: profitable.length > 0 ? 
          profitable.reduce((sum, item) => sum + item.profit, 0) / profitable.length : 0
      }
    };

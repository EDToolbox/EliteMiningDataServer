...
class PerformanceMetricsService extends EventEmitter {
  ...
  recordRequest(endpoint, method, statusCode, responseTime, size = 0) {
    ...
    metric.statusCodes.set(
      statusKey,
      (metric.statusCodes.get(statusKey) || 0) + 1
    );
  ...
  getPerformanceSummary(cutoff) {
    ...
    return {
      ...
      requestsPerSecond: Math.round(
        (totalRequests / (Date.now() - cutoff)) * 1000
      ),
    ...
    for (const [endpoint, metric] of this.requestMetrics) {
      ...
      endpoints.push({
        ...
        requestsPerMinute: Math.round(
          metric.totalRequests / ((Date.now() - metric.firstRequest) / 60000)
        ),
      });
    }
  ...
  getSystemMetrics(cutoff) {
    ...
    const avgMemory =
      recentMetrics.reduce((sum, m) => sum + m.memory.heapUsedPercent, 0) /
      recentMetrics.length;
    const maxMemory = Math.max(
      ...recentMetrics.map((m) => m.memory.heapUsedPercent)
    );
    const avgEventLoopLag =
      recentMetrics.reduce((sum, m) => sum + m.eventLoop.lag, 0) /
      recentMetrics.length;

    return {
      current: {
        memoryUsedPercent:
          Math.round(latest.memory.heapUsedPercent * 100) / 100,
        systemMemoryUsedPercent:
          Math.round(latest.memory.systemUsedPercent * 100) / 100,
  ...
  getPerformanceTrends(cutoff) {
    ...
    for (const [endpoint, metric] of this.requestMetrics) {
      for (const [hourKey, hourStat] of metric.hourlyStats) {
        const hourTime = new Date(
          hourKey.split('-')[0] + ' ' + hourKey.split('-')[1] + ':00:00'
        ).getTime();
    ...
    const trends = Array.from(hourlyData.values())
      ...
      .map((hour) => ({
        ...
        averageResponseTime:
          hour.requests > 0 ? Math.round(hour.totalTime / hour.requests) : 0,
        errorRate:
          hour.requests > 0
            ? Math.round((hour.errors / hour.requests) * 10000) / 100
            : 0,
      }));
    ...
    return {
      ...
      summary: {
        ...
        responseTrend: this.calculateTrend(
          trends.map((t) => t.averageResponseTime)
        ),
  ...
  checkAlerts(endpoint, metric, responseTime, statusCode) {
    ...
    if (responseTime > this.alertThresholds.responseTime) {
      alerts.push({
        type: 'response_time',
        severity:
          responseTime > this.alertThresholds.responseTime * 2
            ? 'critical'
            : 'warning',
        endpoint,
        value: responseTime,
  ...
  toPrometheusFormat(data) {
    ...
    for (const [endpoint, metric] of Object.entries(data.requests)) {
      ...
      output.push(
        `http_request_duration_seconds${labels} ${metric.totalResponseTime / 1000}`
      );

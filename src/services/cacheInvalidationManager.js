...
class CacheInvalidationManager {
  ...
  async invalidateCache(dataType, trigger, context = {}) {
    try {
      ...
      if (!strategy) {
        logger.warn(
          `No invalidation strategy found for data type: ${dataType}`
        );
        return false;
      }
      ...
      logger.info(
        `Invalidating cache for ${dataType} due to ${trigger}`,
        context
      );
  ...
  async invalidateMarketData(trigger, context, strategy) {
    ...
    logger.info(
      `Market data cache invalidated for: ${JSON.stringify(context)}`
    );
  }
  ...
  async invalidateMiningData(trigger, context, strategy) {
    ...
    logger.info(
      `Mining data cache invalidated for: ${JSON.stringify(context)}`
    );
  }
  ...
  async invalidateServerMetrics(trigger, context, strategy) {
    ...
    logger.info(
      `Server metrics cache invalidated for: ${JSON.stringify(context)}`
    );
  }
  ...
  async bulkInvalidation(pattern = '*') {
    ...
    logger.info(
      `Bulk invalidation completed. Total keys invalidated: ${totalInvalidated}`
    );

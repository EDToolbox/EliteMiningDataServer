...
class ErrorHandlingService {
  constructor(config = {}) {
    this.globalConfig = {
      ...
      defaultCircuitBreakerThreshold:
        config.defaultCircuitBreakerThreshold || 5,
      defaultCircuitBreakerTimeout:
        config.defaultCircuitBreakerTimeout || 60000,
  ...
  registerService(serviceName, config = {}) {
    const serviceConfig = {
      ...
      circuitBreaker: {
        threshold:
          ...
          this.globalConfig.defaultCircuitBreakerThreshold,
        timeout:
          config.circuitBreakerTimeout ||
          this.globalConfig.defaultCircuitBreakerTimeout,
        isOpen: false,
        failureCount: 0,
    ...
    logger.info('Registered error handling service', {
      serviceName,
      config: serviceConfig,
    });
  }
  ...
  async executeWithErrorHandling(serviceName, fn, options = {}) {
    ...
    if (!service) {
      throw new Error(
        `Service '${serviceName}' not registered with error handler`
      );
    }
    ...
    for (let attempt = 1; attempt <= maxRetries + 1; attempt++) {
      try {
        ...
        const shouldRetry =
          !isLastAttempt &&
          this.shouldRetryError(service, error, errorType, attempt);
  ...
  shouldRetryError(service, error, errorType, attempt) {
    ...
    if (
      !service.config.retryableErrors.includes(errorType) &&
      errorType !== 'unknown'
    ) {
      return false;
    }
  ...
  calculateRetryDelay(service, attempt, errorType) {
    ...
    const exponentialDelay =
      baseDelay *
      Math.pow(this.globalConfig.exponentialBackoffMultiplier, attempt - 1);
    ...
    const finalDelay = Math.min(
      ...
      this.globalConfig.maxBackoffDelay
    );
  ...
  recordSuccess(service, serviceName, startTime) {
    ...
    if (service.config.circuitBreaker.isOpen) {
      ...
      const recoveryTime =
        Date.now() - service.config.circuitBreaker.lastFailureTime;
  ...
  recordError(service, serviceName, error, errorType) {
    ...
    this.analytics.errorsByType.set(
      errorType,
      (this.analytics.errorsByType.get(errorType) || 0) + 1
    );

    const serviceAnalytics = this.analytics.errorsByService.get(serviceName);
    serviceAnalytics.total++;
    serviceAnalytics.byType.set(
      errorType,
      (serviceAnalytics.byType.get(errorType) || 0) + 1
    );
  ...
  checkCircuitBreakerTimeout(service) {
    ...
    if (now - circuitBreaker.lastFailureTime > circuitBreaker.timeout) {
      ...
      if (
        circuitBreaker.halfOpenRetryCount < circuitBreaker.maxHalfOpenRetries
      ) {
  ...
  updateAverageRecoveryTime(service, recoveryTime) {
    ...
    if (trips === 1) {
      ...
      service.stats.averageRecoveryTime =
        (currentAvg * (trips - 1) + recoveryTime) / trips;
    }
    ...
    if (globalTrips === 1) {
      ...
      this.analytics.averageRecoveryTime =
        (globalAvg * (globalTrips - 1) + recoveryTime) / globalTrips;
    }
  }
  ...
  getGlobalAnalytics() {
    return {
      ...
      errorsByService: Object.fromEntries(
        Array.from(this.analytics.errorsByService.entries()).map(
          ([service, data]) => [
            service,
            {
              ...data,
   ...
   */
  getHealthStatus() {
    const openCircuitBreakers = Array.from(this.services.entries()).filter(
      ([, service]) => service.config.circuitBreaker.isOpen
    );

    const totalRequests = Array.from(this.services.values()).reduce(
      (sum, service) => sum + service.stats.totalRequests,
      0
    );

    const globalErrorRate =
      totalRequests > 0 ? this.analytics.totalErrors / totalRequests : 0;
    ...
    if (
      openCircuitBreakers.length > this.services.size / 2 ||
      globalErrorRate > 0.3
    ) {
      status = 'unhealthy';
    }

...
class RateLimitService {
  ...
  registerService(serviceName, config) {
    ...
    logger.info('Registered rate limiting service', {
      serviceName,
      config: serviceConfig,
    });
  }
  ...
  async makeRequest(serviceName, requestFn, options = {}) {
    ...
    if (!service) {
      throw new Error(
        `Service '${serviceName}' not registered with rate limiter`
      );
    }
  ...
  canMakeRequest(serviceName) {
    ...
    const now = Date.now();
    const adaptiveThrottleFactor =
      this.adaptiveState.throttleFactors.get(serviceName) || 1.0;
    const effectiveMaxRequests = Math.floor(
      service.config.maxRequests / adaptiveThrottleFactor
    );
  ...
  async executeRequest(requestItem) {
    ...
    try {
      ...
      if (service.burstTokens > 0) {
        service.burstTokens--;
        logger.debug('Used burst token', {
          serviceName,
          tokensLeft: service.burstTokens,
        });
      }
  ...
  refillBurstTokens(service, now) {
    ...
    const tokensToAdd = Math.floor(
      timeSinceRefill /
        (service.config.windowMs / service.config.burstAllowance)
    );
  ...
  updateWaitTimeStats(service, waitTime) {
    ...
    this.globalStats.averageWaitTime =
      (globalAvg * (globalTotal - 1) + waitTime) / globalTotal;
  }
  ...
  updateAdaptiveThrottling(serviceName, responseTime, isError) {
    ...
    this.adaptiveState.throttleFactors.set(
      serviceName,
      Math.max(1.0, smoothedFactor)
    );
    ...
    try {
      ...
      for (const priority of this.globalConfig.priorityLevels.reverse()) {
        ...
        while (queue.length > 0) {
          ...
          if (
            Date.now() - requestItem.createdAt >
            this.globalConfig.queueTimeout
          ) {
            clearTimeout(requestItem.timeout);
   ...
   */
  getTotalQueueSize() {
    return Object.values(this.requestQueue).reduce(
  ...
  startAdaptiveThrottling() {
    ...
    setInterval(() => {
      for (const [serviceName, errorData] of this.adaptiveState.errorRates) {
        ...
        const currentFactor =
          this.adaptiveState.throttleFactors.get(serviceName);
        const newFactor = currentFactor * 0.99; // Gradual recovery
        this.adaptiveState.throttleFactors.set(
          serviceName,
          Math.max(1.0, newFactor)
        );
      }
  ...
  getServiceStats(serviceName) {
    ...
    const adaptiveState = {
      ...
      avgResponseTime: this.adaptiveState.responseTimers.get(serviceName).length
        ? this.adaptiveState.responseTimers
            .get(serviceName)
            .reduce((a, b) => a + b, 0) /
          this.adaptiveState.responseTimers.get(serviceName).length
        : 0,
    };
  ...
  getGlobalStats() {
    return {
      ...
      queueByPriority: Object.fromEntries(
        Object.entries(this.requestQueue).map(([priority, queue]) => [
          priority,
          queue.length,
        ])
      ),

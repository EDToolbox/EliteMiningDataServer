...
class InaraApiService {
  constructor(config = {}) {
    ...
    if (!this.apiKey) {
      logger.warn(
        'Inara API initialized without API key - some features will be limited'
      );
    } else {
  ...
  async makeRequest(events, options = {}) {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        const hasErrors = response.data.events.some(
          (event) => event.eventStatus !== 200 && event.eventStatus !== 202
        );

        if (hasErrors) {
          const errorEvents = response.data.events.filter(
            (event) => event.eventStatus !== 200 && event.eventStatus !== 202
          );
  ...
  async getCommodityPrices(commodityName, options = {}) {
    try {
      ...
      logger.error('Failed to get commodity prices', {
        commodityName,
        error: error.message,
      });
  ...
  async getStationMarket(stationId, options = {}) {
    try {
      ...
      logger.error('Failed to get station market', {
        stationId,
        error: error.message,
      });
  ...
  async getStations(searchCriteria, options = {}) {
    try {
      ...
      logger.error('Failed to get stations', {
        searchCriteria,
        error: error.message,
      });
  ...
  async getCommanderProfile(commanderName, options = {}) {
    ...
    try {
      ...
      logger.error('Failed to get commander profile', {
        commanderName,
        error: error.message,
      });
  ...
  async getSquadronInfo(squadronId, options = {}) {
    try {
      ...
      logger.error('Failed to get squadron info', {
        squadronId,
        error: error.message,
      });
  ...
  async batchRequests(events, options = {}) {
    try {
      ...
      logger.error('Failed to execute batch requests', {
        error: error.message,
      });
      ...
      averageRequestsPerMinute:
        this.requestHistory.length > 0
          ? this.requestHistory.length /
            ((now - this.requestHistory[0]) / 60000)
          : 0,
      authenticated: !!this.apiKey,

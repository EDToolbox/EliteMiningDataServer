...
const calculateDistance = (coord1, coord2) => {
  if (
    !coord1 ||
    !coord2 ||
    typeof coord1.x !== 'number' ||
    typeof coord1.y !== 'number' ||
    typeof coord1.z !== 'number' ||
    typeof coord2.x !== 'number' ||
    typeof coord2.y !== 'number' ||
    typeof coord2.z !== 'number'
  ) {
    return 0;
  }
  ...
  try {
    ...
    if (req.services.cache) {
      try {
        ...
        if (cachedData) {
          logger.debug('Cache hit for commodity data', {
            commodityId,
            cacheKey,
          });
          return res.json({
    ...
    const eddnMarketData = await req.services.marketData.getCommodityData(
      commodityId,
      options
    );
    ...
    if (includeInaraData === 'true' && req.services.inara) {
      try {
        ...
        if (req.services.errorHandler) {
          ...
          inaraData = await req.services.inara.getCommodityPrices(
            commodityId,
            inaraOptions
          );
        }
    ...
    if (includeHistory === 'true' || includeTrends === 'true') {
      try {
        const historyData =
          await req.services.marketData.getCommodityPriceHistory(commodityId, {
            systemName,
            days: 30,
            intervals:
              includeTrends === 'true' ? ['daily', 'weekly'] : ['daily'],
          });
    ...
    if (req.services.cache) {
      try {
        const cacheTimeout =
          result.metadata.dataFreshness.averageAge < 3600000 ? 300 : 600; // 5-10 minutes
        await req.services.cache.set(cacheKey, result, cacheTimeout);
        result.metadata.cacheStatus = 'stored';
        logger.debug('Commodity data cached successfully', {
          commodityId,
          cacheTimeout,
        });
      } catch (cacheError) {
  ...
  try {
    ...
    try {
      if (req.services.edsm) {
        ...
        startSystemData =
          await req.services.marketData.getSystemCoordinates(startSystem);
        if (!startSystemData) {
          return res.status(503).json({
            success: false,
            error: 'Service unavailable',
            message:
              'EDSM service not available and system not in local database',
          });
        }
      }
    } catch (coordError) {
    ...
    if (includeVisualization === 'true') {
      visualizationData = generateRouteVisualization(
        tradingRoutes,
        startSystemData
      );
    }
    ...
    const response = {
      ...
      statistics: routeStatistics,
      visualization:
        includeVisualization === 'true' ? visualizationData : undefined,
      metadata: {
...
async function getNearbyTradingSystems(
  startCoords,
  jumpRange,
  services,
  options = {}
...
async function getMarketDataForSystems(systems, services, options = {}) {
  try {
    ...
    const filteredMarketData = marketData.filter((data) => {
      ...
      if (shipSize === 'large' && data.maxLandingPadSize === 'Small')
        return false;
      if (shipSize === 'large' && data.maxLandingPadSize === 'Medium')
        return false;

      // Filter by station type
      if (stationType === 'orbital' && data.stationType?.includes('Planetary'))
        return false;
      if (
        stationType === 'planetary' &&
        !data.stationType?.includes('Planetary')
      )
        return false;
    ...
    return {
      ...
      totalCommodities: [
        ...new Set(
          filteredMarketData.flatMap((data) =>
...
async function calculateTradingProfits(marketData, options = {}) {
  try {
    ...
    for (const buyStation of marketData) {
      ...
      for (const [commodity, buyData] of Object.entries(
        buyStation.commodities
      )) {
        ...
        for (const sellStation of marketData) {
          if (
            sellStation.system === buyStation.system &&
            sellStation.station === buyStation.station
          )
            continue;
...
async function generateOptimalRoutes(
  startSystem,
  profitOpportunities,
  systemsData,
  options = {}
  ...
  try {
    ...
    for (const opportunity of profitOpportunities.slice(0, 200)) {
    ...
    routes.sort((a, b) => {
      switch (sortBy) {
        ...
        case 'efficiency':
          return (
            b.profit.total / b.totalDistance - a.profit.total / a.totalDistance
          );
        default:
          return b.profit.total - a.profit.total;
      }
...
function generateMultiHopRoutes(
  startSystem,
  profitOpportunities,
  systemCoords,
  options
    ...
    for (let i = 0; i < Math.min(profitOpportunities.length, 50); i++) {
      ...
      for (let j = 0; j < Math.min(profitOpportunities.length, 20); j++) {
        ...
        const leg1 = calculateDistance(
          startSystem.coords,
          systemCoords[firstTrade.buy.system]
        );
        const leg2 = calculateDistance(
          systemCoords[firstTrade.buy.system],
          systemCoords[firstTrade.sell.system]
        );
        const leg3 = calculateDistance(
          systemCoords[secondTrade.buy.system],
          systemCoords[secondTrade.sell.system]
        );
        ...
        const estimatedTime = calculateRouteTime(
          [
            { system: startSystem.name, coords: startSystem.coords },
            {
              system: firstTrade.buy.system,
              coords: systemCoords[firstTrade.buy.system],
            },
            {
              system: firstTrade.sell.system,
              coords: systemCoords[firstTrade.sell.system],
            },
            {
              system: secondTrade.sell.system,
              coords: systemCoords[secondTrade.sell.system],
            },
          ],
          options
        );
        ...
        multiHopRoutes.push({
          ...
          profit: {
            ...
            investment:
              firstTrade.buy.price * firstTrade.profit.maxCargo +
              secondTrade.buy.price * secondTrade.profit.maxCargo,
          },
          risk: {
            level: 'Unknown',
            factors: [],
...
function calculateRouteStatistics(routes, metadata) {
  ...
  const totalProfit = routes.reduce(
    (sum, route) => sum + route.profit.total,
    0
  );
  ...
  const riskDistribution = routes.reduce(
    (acc, route) => {
...
router.get('/trends', async (req, res) => {
  try {
    ...
    const aggregatedData = aggregatePriceDataByInterval(
      historicalData,
      interval
    );
    ...
    let visualizationData = null;
    if (includeVisualization === 'true') {
      visualizationData = generateTrendVisualization(aggregatedData, {
        chartType,
        movingAverages,
...
async function queryHistoricalMarketData(params, services) {
  try {
    const { commodity, system, region, startDate, endDate, interval, limit } =
      params;
...
function aggregatePriceDataByInterval(data, interval) {
  ...
  return Object.values(aggregated)
    .map((bucket) => ({
      ...
      averageSupply:
        bucket.supplies.reduce((sum, s) => sum + s, 0) /
          bucket.supplies.length || 0,
      averageDemand:
        bucket.demands.reduce((sum, d) => sum + d, 0) / bucket.demands.length ||
        0,
...
function calculateEMA(data, period) {
  ...
  for (let i = period - 1; i < data.length; i++) {
    const currentEMA =
      data[i].close * multiplier + previousEMA * (1 - multiplier);
    ema.push({
      timestamp: data[i].timestamp,
      value: currentEMA,
    });
...
function analyzeWeeklyPatterns(data) {
  ...
  return Object.values(weeklyData).map((day) => ({
    ...
    averageVolume:
      day.volumes.reduce((sum, v) => sum + v, 0) / day.volumes.length,
...
function calculateVolatilityIndicators(data, options) {
  const { includeATR, includeBollingerBands, includeRSI, includeStochastics } =
    options;
...
async function generateTrendPredictions(data, options) {
  const { forecastPeriods, confidenceIntervals, includeSeasonality, method } =
    options;
  ...
  return {
    ...
    method,
    confidence:
      trend.rSquared > 0.5 ? 'high' : trend.rSquared > 0.3 ? 'medium' : 'low',
  };
}
...
function determineCycleLength(interval) {
  switch (interval) {
    ...
    case 'weekly':
      return 4; // Monthly cycle
    case 'monthly':
      return 12; // Yearly cycle
    default:
      return 7;
  }
}
...
function determineForecastPeriods(interval) {
  switch (interval) {
    case 'hourly':
      return 24; // Next 24 hours
    case 'daily':
      return 7; // Next 7 days
    case 'weekly':
      return 4; // Next 4 weeks
    case 'monthly':
      return 3; // Next 3 months
    default:
      return 7;
  }
}
...
function calculatePredictionMargin(values, confidenceLevel) {
  ...
  const factor =
    confidenceLevel === 95 ? 1.96 : confidenceLevel === 80 ? 1.28 : 1.0;
...
function sendTrendsCSVResponse(res, data, commodity) {
  ...
  res.setHeader(
    'Content-Disposition',
    `attachment; filename="${commodity || 'market'}_trends.csv"`
  );
  res.send(csv);
}
...
function calculateDataQuality(data) {
  ...
  const missingValues = data.filter(
    (item) => !item.buyPrice && !item.sellPrice
  ).length;
  if (missingValues > 0) {
...
router.get('/station/:systemName/:stationName', async (req, res) => {
  try {
    ...
    logger.info(
      `Station market data requested: ${stationName} in ${systemName}`
    );

    const stationData = await marketDataService.getStationMarketData(
      systemName,
      stationName
    );
...
async function getCommodityCategory(commodityId, services) {
  try {
    ...
    const commodityInfo =
      await services.marketData.getCommodityInfo(commodityId);
    ...
    logger.warn('Failed to get commodity category', {
      commodityId,
      error: error.message,
    });
    return 'Unknown';
...
async function getCommodityRarity(commodityId, services) {
  try {
    ...
    logger.warn('Failed to get commodity rarity', {
      commodityId,
      error: error.message,
    });
    return 'Common';
...
function calculateDataFreshness(locations) {
  ...
  const freshnessScore = Math.max(
    0,
    100 - (averageAge / maxAcceptableAge) * 100
  );
...
function calculatePriceAnalysis(eddnLocations, inaraData) {
  ...
  const buyPrices = allLocations
    .map((loc) => loc.prices?.buy || loc.buyPrice || 0)
    .filter((p) => p > 0);
  const sellPrices = allLocations
    .map((loc) => loc.prices?.sell || loc.sellPrice || 0)
    .filter((p) => p > 0);
    ...
    sell:
      sellPrices.length > 0
        ? {
            ...
            average:
              sellPrices.reduce((sum, p) => sum + p, 0) / sellPrices.length,
...
function calculateProfitOpportunities(locations, options = {}) {
  ...
  const buyStations = locations.filter(
    (loc) => (loc.prices?.buy || loc.buyPrice) > 0 && (loc.supply || 0) > 0
  );

  const sellStations = locations.filter(
    (loc) => (loc.prices?.sell || loc.sellPrice) > 0 && (loc.demand || 0) > 0
  );
  ...
  for (const buyStation of buyStations) {
    for (const sellStation of sellStations) {
      if (
        buyStation.system === sellStation.system &&
      ...
      if (profit >= minProfit) {
        opportunities.push({
          ...
          profit: {
            ...
            potential:
              Math.min(buyStation.supply || 0, sellStation.demand || 0) *
              profit,
          },
        });
      }
...
function calculateMarketVolatility(locations, priceHistory) {
  ...
  const buyPrices = locations
    .map((loc) => loc.prices?.buy || loc.buyPrice || 0)
    .filter((p) => p > 0);
  const sellPrices = locations
    .map((loc) => loc.prices?.sell || loc.sellPrice || 0)
    .filter((p) => p > 0);
  ...
  const priceVariation =
    buyPrices.length > 0
      ? calculateStandardDeviation(buyPrices) /
        (buyPrices.reduce((sum, p) => sum + p, 0) / buyPrices.length)
      : 0;

  const supplyVariation =
    supplies.length > 0
      ? calculateStandardDeviation(supplies) /
        (supplies.reduce((sum, s) => sum + s, 0) / supplies.length)
      : 0;

  const demandVariation =
    demands.length > 0
      ? calculateStandardDeviation(demands) /
  ...
  const volatilityScore = Math.min(
    100,
    ((priceVariation + supplyVariation + demandVariation) * 100) / 3
  );
...
async function calculateRegionalComparison(
  commodityId,
  centerSystem,
  services,
  maxDistance
  ...
  try {
    ...
    return {
      ...
      regional: {
        averageBuyPrice:
          systemStats.reduce((sum, s) => sum + s.averageBuyPrice, 0) /
          systemStats.length,
        averageSellPrice:
          systemStats.reduce((sum, s) => sum + s.averageSellPrice, 0) /
          systemStats.length,
...
function sendCSVResponse(res, result, commodityId) {
  ...
  let csv =
    'System,Station,Buy Price,Sell Price,Supply,Demand,Distance,Last Updated\n';
  ...
  res.setHeader(
    'Content-Disposition',
    `attachment; filename="${commodityId}_market_data.csv"`
  );
  res.send(csv);
}
...
router.get('/station/:system/:station', ensureServices, async (req, res) => {
  ...
  try {
    ...
    const validationResults = await validateSystemAndStation(
      system,
      station,
      req.services
    );
    ...
    if (includeInaraData && req.services.inara) {
      try {
        ...
        logger.warn('Inara API error for station data', {
          error: error.message,
        });
      }
    }
    ...
    const sortedCommodities = sortStationCommodities(
      enrichedCommodities,
      sortBy
    );

    // Apply limit
    const limitedCommodities = limit
      ? sortedCommodities.slice(0, limit)
      : sortedCommodities;
    ...
    const response = {
      ...
      statistics: {
        ...
        profitableCommodities: profitAnalysis
          ? profitAnalysis.profitable.length
          : 0,
  ...
  try {
    ...
    if (services.edsm) {
      try {
        ...
        logger.warn('EDSM validation failed', {
          system: systemName,
          error: error.message,
        });
      }
    ...
    const stationExists = await services.marketData.checkStationExists(
      systemName,
      stationName
    );
    if (!stationExists) {
      const suggestions = await services.marketData.findSimilarStations(
        systemName,
        stationName,
        5
      );
...
async function fetchStationCommodities(
  system,
  station,
  filterCommodities,
  services
  ...
  try {
    ...
    const commodities = await services.marketData
      .find(filter)
      .sort({ timestamp: -1 });
    ...
    commodities.forEach((item) => {
      if (
        !latestCommodities[item.commodity] ||
...
async function enrichCommodityData(commodityData, station, services) {
  try {
    return commodityData.map((commodity) => {
      const ageHours =
        (Date.now() - new Date(commodity.timestamp)) / (1000 * 60 * 60);
...
async function fetchStationInformation(
  ...
  try {
    ...
    const info = {
      ...
      landingPads: {
        large:
          stationInfo?.landingPadSize === 'L' ||
          stationInfo?.landingPadSize === 'Mixed',
...
async function calculateStationProfitOpportunities(
  commodities,
  system,
  station,
  minProfit,
  ...
  try {
    ...
    return {
      ...
      summary: {
        ...
        averageProfit:
          profitable.length > 0
            ? profitable.reduce((sum, item) => sum + item.profit, 0) /
              profitable.length
            : 0,
      },
    };
  } catch (error) {
    logger.error('Profit opportunities calculation error', {
      error: error.message,
    });
    return null;
  }
...
async function compareWithNearbyStations(
  system,
  station,
  commodities,
  services
      ...
      for (const nearbyStation of nearbyStations.slice(0, 5)) {
        ...
        if (nearbyData) {
          comparison.nearbyStations.push({
            ...
            sellPriceDifference:
              (nearbyData.sellPrice || 0) - commodity.sellPrice,
          });
        }
      }
  ...
  try {
    ...
    const averageAge =
      commodities.length > 0
        ? totalAge / commodities.length / (1000 * 60 * 60)
        : 0;
    const stalePercentage =
      commodities.length > 0 ? (staleCount / commodities.length) * 100 : 0;
    ...
    logger.error('Market data freshness calculation error', {
      error: error.message,
    });
    return {
      score: 0,
...
function sortStationCommodities(commodities, sortBy) {
  try {
    switch (sortBy) {
      ...
      case 'price_asc':
        return commodities.sort(
          (a, b) => (a.sellPrice || 0) - (b.sellPrice || 0)
        );
